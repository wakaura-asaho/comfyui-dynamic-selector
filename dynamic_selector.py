    ...
    def _check_type_consistency(cls, **kwargs) -> bool | str:
        ...
        for key in input_keys:
            ...
            if first_type is None:
                ...
                raise TypeError(f"DynamicGroupSelector: Inconsistent types: {key} is '{current_type}', but {first_key} is '{first_type}'.")
        
        return True

    @classmethod
    def fingerprint_inputs(cls, select_group: int, index: int, type_strict: bool, **kwargs) -> tuple[int, int]:
        return (select_group, index)

    @classmethod
    def validate_inputs(cls, select_group: int, index: int, type_strict: bool, **kwargs) -> bool | str:
        group_key = f"input_{select_group}"
        if group_key not in kwargs:
            return f"Input '{group_key}' must be connected for selection {select_group}." 
            
        return True

    @classmethod
    def execute(cls, select_group: int, index: int, type_strict: bool, **kwargs) -> io.NodeOutput:
        ...
        type = group["type"] if isinstance(group, dict) and "type" in group else "plain_data"
        count = len(data) if data is not None else 0

        print(f"E: {data}, type: {type}, len: {count}")

        valid = type_strict and cls._check_type_consistency(**kwargs) or not type_strict
        if valid:
            if data is None:
                raise ValueError(f"DynamicGroupSelector: GROUP input '{group_key}' is not connected.")
            if index < 0 or index >= count:
                raise IndexError(f"DynamicGroupSelector: Index {index} is out of bound. The group {select_group} has {count} item(s).")
...
class DynamicTypeSelector(io.ComfyNode):
    ...
    @classmethod
    def validate_inputs(cls, select: int, use_bool_item: bool, bool_item: bool, item_true: int, item_false: int, **kwargs) -> bool | str:
        selected_index = select if not use_bool_item else (item_true if bool_item else item_false)
    ...
    def execute(cls, select: int, use_bool_item: bool, bool_item: bool, item_true: int, item_false: int, **kwargs) -> io.NodeOutput:
        index = select if not use_bool_item else (item_true if bool_item else item_false)
        input_keys = [k for k in kwargs.keys() if k.startswith("input_")]
        count = len(input_keys)

        if index < 0 or index >= count:
            raise IndexError(f"DynamicTypeSelector: Index {index} is out of bound. The node has {count} input(s).")

        input_key = f"input_{index}"
        val = kwargs.get(input_key, None)
        if val is None:
            raise ValueError(f"DynamicTypeSelector: Selected input '{input_key}' is missing or not connected.")
...
class SchemaDefineHelper:
    @staticmethod
    def selection_input(max_inputs: int = define.max_inputs, id: str = "select", tooltip: str = "") -> io.Int.Input:
        return io.Int.Input(
            id=id,
            display_name=id,
            default=0,
    ...
    def dynamic_output(id: str = "output") -> io.AnyType.Output:
        return io.AnyType.Output(
            id=id,
            display_name=id.upper()
        )
